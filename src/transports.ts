...
import { levelWeights, safeStringify } from "./utils";
...
export class DiscordTransport implements LogTransport {
    ...
    private shouldLog(entry: LogEntry): boolean {
        if (entry.meta?.discord === true) return true; // Force Send
        if (entry.meta?.discord === false) return false; // Force Skip
   if (this.options.customCheck && !this.options.customCheck(entry)) {
        ...
        if (
            filter.onlyLabels &&
            (!entry.label || !filter.onlyLabels.includes(entry.label))
        ) {
            return false;
        }
        ...
        if (
            filter.includeLevels &&
            filter.includeLevels.includes(entry.level)
        ) {
            return true;
        }

        const minLevel = filter.minLevel || "error";
        const entryWeight = levelWeights[entry.level as LogLevel] ?? 0;
